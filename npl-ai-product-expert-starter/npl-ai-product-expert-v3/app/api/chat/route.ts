...
function isValidMode(mode: string): mode is AgentMode {
  return ["speaker", "lesson", "training", "panel", "sparring", "frontier"].includes(mode);
}
...
export async function POST(req: Request) {
  try {
    ...
    // Optional and OFF by default for cost/source-control reasons.
    // Only turn this on later after testing the exact supported tool config for your OpenAI account.
    // if (process.env.ENABLE_WEB_SEARCH === "true" && mode === "frontier") {
